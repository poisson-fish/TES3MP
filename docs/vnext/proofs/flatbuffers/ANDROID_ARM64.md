...
the pinned source. No Quest or Android production job is added before the
post-release feasibility decision.

The Android ARM64 feasibility conclusion is **supported in principle, not yet a
release gate**. A later feasibility milestone must still cross-compile the
then-current pinned runtime and generated vNext schemas with its selected NDK.
Failure there reopens the codec decision before any target-specific workaround
may leak into protocol types.
