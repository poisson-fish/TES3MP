...
| 6.3 | Implement required encryption, optional join-password authentication, resume-token handling, and credential redaction per ADR-0005 | **Not Started** | Proposed [`ADR-0034`](adr/ADR-0034-phase6-credential-and-resumption-boundary.md) awaits owner approval; encryption ordering, unencrypted-mode rejection, password failure/rate-limit, token replay, timeout, and redaction tests must pass after approval |
