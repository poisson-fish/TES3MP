...
| 6.3 | Implement required encryption, optional join-password authentication, resume-token handling, and credential redaction per ADR-0005 | **In Progress** | Owner-approved [`ADR-0034`](adr/ADR-0034-phase6-credential-and-resumption-boundary.md); bounded credential codecs/values and the move-only admission result plus bounded digest-only transactional resume store pass applicable local contracts; password-provider, rate-limit, private-crypto, transport scope, composition, redaction, and loopback closure remain |
