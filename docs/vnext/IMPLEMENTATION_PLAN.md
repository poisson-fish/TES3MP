...
| 6.5 | Implement network telemetry and stable disconnect/rejection reasons | **In Progress** | Owner-approved [`ADR-0038`](adr/ADR-0038-phase6-network-telemetry-and-stable-reasons.md) selects transport-owned closed telemetry, stable reasons, bounded cardinality, sink isolation, and honest per-lane retransmission-pressure gauges; queue/runtime emission, saturation, sink isolation, and public GNS lane-pressure sampling pass focused local verification; owner implementation-demo acceptance remains |
