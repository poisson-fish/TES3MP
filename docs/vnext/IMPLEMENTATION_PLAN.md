...
| 6.3 | Implement required encryption, optional join-password authentication, resume-token handling, and credential redaction per ADR-0005 | **In Progress** | Owner-approved [`ADR-0034`](adr/ADR-0034-phase6-credential-and-resumption-boundary.md); bounded T3AQ/T3AA/T3AR codecs, move-only credential/token values, exact schema generation, corpus, fuzz registration, and applicable local contracts pass; password-provider, rate-limit, resume-store, private-crypto, composition, redaction, and loopback closure remain |
