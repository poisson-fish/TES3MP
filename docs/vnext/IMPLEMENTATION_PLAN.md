...
| 6.3 | Implement required encryption, optional join-password authentication, resume-token handling, and credential redaction per ADR-0005 | **In Progress** | Owner-approved [`ADR-0034`](adr/ADR-0034-phase6-credential-and-resumption-boundary.md); bounded credential codecs/values, move-only admission, digest-only transactional resume store, fixed-work optional join-password provider, opaque admission scope, bounded global/source limiter, and private exact-pinned OpenSSL 3.5.8 credential crypto pass applicable local contracts; transport scope derivation, composition, redaction, and loopback closure remain |
