...
| 7.7 | Add adverse-network matrix, reconnect loop, queue-bound, and soak tests | **In Progress** | Accepted [`ADR-0046`](adr/ADR-0046-phase7-adverse-network-and-soak-matrix.md); typed profile catalog and seeded 10,000-tick deterministic matrix pass bounded replay, reliable apply-once/order, latest-wins convergence, stall, and zero-drain contracts; reconnect loop is next |
