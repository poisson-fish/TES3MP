...
| 8.4 | Map interior/exterior cell changes and spawn/despawn remote player presentation | **In Progress** | Explicit snapshot identity and tracked/coalesced cell transitions remain verified. The ADR-0050/GDR-0013/P8-004 remote-avatar architecture is reopened after the real-content failure and legacy audit showed that the nominal renderer-only seam initializes full NPC gameplay data; owner architecture review, correction, automated gates, and two-client demo remain |
