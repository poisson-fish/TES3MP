...
| 6.3 | Implement required encryption, optional join-password authentication, resume-token handling, and credential redaction per ADR-0005 | **In Progress** | Owner-approved [`ADR-0034`](adr/ADR-0034-phase6-credential-and-resumption-boundary.md) and [`ADR-0035`](adr/ADR-0035-phase6-transport-admission-scope-handoff-and-derivation.md); bounded credential codecs/values, move-only admission, digest-only transactional resume store, fixed-work optional join-password provider, opaque admission scope, bounded global/source limiter, private exact-pinned OpenSSL 3.5.8 credential crypto, and transport accepted-event HMAC scope derivation pass applicable local contracts; composition, redaction, and complete authentication loopback closure remain |
