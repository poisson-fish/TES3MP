...
| 7.3 | Implement authentication, session creation, player creation, and join | **In Progress** | Accepted [`ADR-0042`](adr/ADR-0042-phase7-authenticated-join-and-identity-allocation.md); bounded prepare/token/frame/atomic transport-pair enqueue/commit composition, preissued initial-session binding, app-local connection ownership, resume-context derivation, and bounded per-connection frame dispatch pass; transport receive/pump wiring and two-client process demo remain |
