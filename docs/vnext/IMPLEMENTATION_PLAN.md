...
| 6.4 | Implement bounded queues, priority, rate limits, backpressure, and slow-peer eviction | **In Progress** | Owner-approved [`ADR-0037`](adr/ADR-0037-phase6-bounded-outbound-queue-and-slow-peer-policy.md) fixes per-connection reliable/latest retention, fair servicing, deterministic rate limits, hard ceilings, and sustained reliable-block eviction; implementation and demo evidence remain |
