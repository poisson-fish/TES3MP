...
| 8.5 | Convert input to semantic movement commands and apply authoritative local correction | **Not Started** | Decision review pending: exact entity-revision preconditions cannot converge for continuous movement after more than one tick of latency; command concurrency, desktop mapping, and local-correction behavior require owner approval before code |
