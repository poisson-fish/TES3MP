...
| 8.2 | Implement the adapter lifecycle and desktop input/presentation provider interfaces | **In Progress** | Accepted ADR-0047 governs provider/runtime composition; the headless executable now uses the reusable bounded runtime; accepted ADR-0048 canonical-revision domain/wire migration and OpenMW composition remain |
