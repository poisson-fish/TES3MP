...
namespace
{
    ...
    const char* describe(TES3MP::OpenMWAdapter::ConnectionStatus status) noexcept
    {
        ...
        switch (status)
        {
            ...
            case Status::Reconnecting:
                return "server disconnected; attempting session resume";
            case Status::Resumed:
    ...
    class MultiplayerStatus final : public TES3MP::OpenMWAdapter::ConnectionStatusProvider
    {
        ...
        {
            using Status = TES3MP::OpenMWAdapter::ConnectionStatus;
            const std::string prefix = status == Status::Reconnecting || status == Status::Resumed
                ? "TES3MP connection status: "
                : "TES3MP connection stopped: ";
            const std::string message = prefix + describe(status);
...
)
{
    ...
    if (variables["tes3mp-enable"].as<bool>())
    {
        ...
        std::vector<TES3MP::OpenMWAdapter::DesktopSpellMapping> spellMappings;
        for (const auto& entry : variables["tes3mp-content-spell-map"].as<StringsVector>())
        {
            const auto equal = entry.find('=');
            std::uint64_t rawId = 0;
            const auto parsed = equal == std::string::npos ? std::from_chars_result{}
                                                           : std::from_chars(entry.data(), entry.data() + equal, rawId);
            const auto id = equal != std::string::npos && parsed.ec == std::errc{} && parsed.ptr == entry.data() + equal
                ? TES3MP::SpellRecordId::fromValue(rawId)
                : std::nullopt;
            if (!id || equal + 1 == entry.size())
            {
                Log(Debug::Error) << "TES3MP startup failed: invalid spell mapping";
                return false;
            }
            spellMappings.push_back({ *id, entry.substr(equal + 1) });
        }
        auto contentMapping = TES3MP::OpenMWAdapter::DesktopContentMapping::create(*contentManifest, localMappings,
            ...
            dialogueChoiceMappings, weatherRegionMappings, weatherMappings, spellMappings);
        if (!contentMapping)
        {
        ...
        TES3MP::OpenMWAdapter::ConnectionStatusProvider* status = &multiplayerStatus;
        TES3MP::OpenMWAdapter::ConnectionControlProvider* control = nullptr;
#ifdef TES3MP_OPENMW_DESKTOP_AUTOMATION
        const auto automationRole = variables["tes3mp-automation-role"].as<std::string>();
        const auto automationOutput = variables["tes3mp-automation-output"].as<Files::MaybeQuotedPath>().u8string();
        if (!automationRole.empty() || !automationOutput.empty())
        {
            const auto role = TES3MP::OpenMWAdapter::parseDesktopAutomationRole(automationRole);
            if (!role || automationOutput.empty())
            {
                Log(Debug::Error) << "TES3MP startup failed: automation role or output is invalid";
                return false;
            }
            ...
            input = multiplayerAutomation.get();
            presentation = multiplayerAutomation.get();
            status = multiplayerAutomation.get();
