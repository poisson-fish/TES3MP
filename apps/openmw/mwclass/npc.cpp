    ...
    void Npc::ensureCustomData(const MWWorld::Ptr& ptr) const
    {
        if (!ptr.getRefData().getCustomData())
        {
            ...
            else
            {
                ...
                const auto& content = *MWBase::Environment::get().getESMStore();
                data->mNpcStats.initializeAutoStats(*ref->mBase, content);
                if (!spellsInitialised)
                    data->mNpcStats.getSpells().addAllToInstance(MWMechanics::autoCalcNpcSpells(
                        data->mNpcStats.getSkills(), data->mNpcStats.getAttributes(),
                        content.get<ESM::Race>().find(ref->mBase->mRace), content));
