...
namespace MWWorld
{
    void Calendar::setup(Globals& globalVariables)
    {
    ...
    void Calendar::setHour(double hour)
    {
        if (hour < 0)
            hour = 0;
    ...
    TimeStamp Calendar::getTimeStamp() const
    {
        return TimeStamp(mGameHour, mDaysPassed);
    }

    ESM::EpochTimeStamp Calendar::getEpochTimeStamp() const
    {
    ...
    void Calendar::advanceTime(double hours, Globals& globalVariables)
    {
        hours += mGameHour;
        setHour(hours);
    ...
    void Calendar::updateGlobalFloat(GlobalVariableName name, float value)
    {
    ...
    void Calendar::updateGlobalInt(GlobalVariableName name, int value)
    {
        if (name == Globals::sGameHour)
        {
