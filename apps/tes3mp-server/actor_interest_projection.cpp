...
namespace TES3MP::ServerApp
{
    namespace
    {
        ...
        std::optional<std::vector<ActorSpatialSnapshot>> visibleEntries(
            const CanonicalServerState& players, const CanonicalActorWorld& actors, SessionId target, ServerTick tick)
        {
            ...
            for (const auto& actor : actors.actors())
                if (actor.root().cell() == player->transform().cell())
                    entries.emplace_back(tick, actor.actorId(), actor.entityId(), actor.prototypeId(), actor.revision(),
                        actor.authorityEpoch(), actor.root(), actor.velocity(), actor.activity());
            return entries;
        }
    }
    ...
    try
    {
        ...
        if (!std::holds_alternative<ReliableActorInterestBaseline>(baseline)
            || !std::holds_alternative<ActorWorldView>(view))
            return std::nullopt;
        return ActorInterestBaselineDelivery{ target, std::get<ReliableActorInterestBaseline>(std::move(baseline)),
    ...
    try
    {
        ...
        if (!session || !entries)
            return std::nullopt;
        auto view = ActorWorldView::create(*entries);
        if (!std::holds_alternative<ActorWorldView>(view))
            return std::nullopt;
        return LatestWinsActorSnapshot(
            target, session->sessionGeneration(), tick, canonicalRevision, std::get<ActorWorldView>(std::move(view)));
    }
    catch (...)
    {
        return std::nullopt;
    ...
    {
        auto baseline = encodeProtocolFrame(MessageClass::ReliableOperation, MessageKind::ReliableActorInterestBaseline,
            encodeReliableActorInterestBaseline(delivery.baseline));
        auto view = encodeProtocolFrame(MessageClass::LatestWinsSnapshot, MessageKind::LatestWinsActorSnapshot,
            encodeLatestWinsActorSnapshot(delivery.view));
        if (!std::holds_alternative<std::vector<std::byte>>(baseline)
            || !std::holds_alternative<std::vector<std::byte>>(view))
            return false;
        return queues.enqueuePair(connection, TransportChannel::ReliableOrdered,
                   ...
                   std::get<std::vector<std::byte>>(view))
    ...
    try
    {
        ...
        {
            (void)connection;
            auto frame = encodeProtocolFrame(MessageClass::LatestWinsSnapshot, MessageKind::LatestWinsActorSnapshot,
                encodeLatestWinsActorSnapshot(view));
