...
namespace TES3MP::Native
{
    namespace
    {
        ...
        uint64_t magic(size_t count, bool world) { return world ? WorldMagic : count == 0 ? PairMagic : count == 1 ? SharedMagic : CellMagic; }
        size_t headerSize(size_t count, bool world) { return 32 + 8 * (count + world) + (count > 1 || world ? 8 : 0); }
        void validate(const EquipmentSessionValues& values, const std::array<EquipmentBindings, 2>& bindings,
            std::span<const EquipmentBindings> containers, const EquipmentBindings* world)
        {
    ...
    void encodeEquipmentSession(const EquipmentSessionValues& values,
        ...
        std::span<const EquipmentBindings> containers, const EquipmentBindings* world)
    {
        validate(values, bindings, containers, world);
        std::vector<EquipmentBytes> owners(2 + containers.size() + (world != nullptr));
        size_t size = headerSize(containers.size(), world != nullptr);
        for (size_t i = 0; i < owners.size(); ++i)
        {
            const bool ground = i == 2 + containers.size();
            encodeEquipment(ground ? *values.mWorldItems : i < 2 ? values.mActors[i] : values.mContainers[i - 2],
        ...
        staged.reserve(size);
        put(staged, magic(containers.size(), world != nullptr));
        ...
        std::span<const EquipmentBindings> containers, const EquipmentBindings* world)
    {
        ...
        const auto header = headerSize(containers.size(), world != nullptr);
        if (bytes.size() < header || bytes.size() > MaxEquipmentSessionBytes
            || get(bytes, 0) != magic(containers.size(), world != nullptr)
        ...
        std::array<size_t, 3 + MaxEquipmentContainers> lengths{};
        ...
        validate(staged, bindings, containers, world);
        output.swap(staged);
    }
}
