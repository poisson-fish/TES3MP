...
namespace TES3MP::Native
{
    namespace
    {
        ...
        FileReadResult openRead(
            File& file, const std::filesystem::path& path, uint64_t& size, size_t maximum, FileFaults& faults) noexcept
        {
            if (faults.mFail == FileFault::ReadOpen || !file.open(path) || faults.mFail == FileFault::ReadSize
                || !file.size(size))
                return FileReadResult::Unavailable;
            return size > maximum ? FileReadResult::TooLarge : FileReadResult::Read;
        }
        ...
        bool verifyFile(const std::filesystem::path& path, std::span<const char> bytes, size_t maximum,
            FileFaults& faults) noexcept
        {
            File file;
            uint64_t size = 0;
            if (openRead(file, path, size, maximum, faults) != FileReadResult::Read || size != bytes.size())
                return false;
        ...
        const std::filesystem::path& path, size_t maximum, std::vector<char>& output, FileFaults& faults)
    {
        File file;
        uint64_t size = 0;
        const auto result = openRead(file, path, size, maximum, faults);
        if (result != FileReadResult::Read)
            return result;
        std::vector<char> staged(static_cast<size_t>(size));
    ...
    BoundedFileSink::BoundedFileSink(const std::filesystem::path& path, size_t maximum)
        : mPath(path)
        , mTemporary(path)
        , mParent(path.parent_path())
        , mMaximum(maximum)
    {
        if (path.empty() || mParent.empty() || maximum == 0)
            throw std::invalid_argument("Bounded file sink requires a scratch directory and positive bound");
        mTemporary += ".tmp";
    }
