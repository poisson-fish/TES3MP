...
namespace MWWorld::Testing
{
    namespace
    {
        ...
        InventoryViewSnapshot savedViews(const SerializedPair& saved, const SaveEnvelope& envelope)
        {
            const auto owned = [](ESM::RefNum id) { return InventoryInstanceId{ id.mIndex, id.mContentFile }; };
            InventoryViewSnapshot result;
            for (size_t side = 0; side < 2; ++side)
            {
                const auto& inventory = side == 0 ? saved.mSource : saved.mDestination;
                auto& view = result.mOwners[side];
                view.mOwner = owned(side == 0 ? envelope.mSourceOwner : envelope.mDestinationOwner);
                view.mSelection = owned(inventory.mSelection);
                view.mRevision = saved.mRestart.mRevision;
                for (size_t i = 0; i < inventory.mObjects.size(); ++i)
                    view.mItems.push_back({ owned(inventory.mProposedIdentities[i]), inventory.mObjects[i].mRef.mCount });
            }
            return result;
        }

        auto viewStorage(const InventoryViewSnapshot& views)
        {
            return std::tuple{ views.mOwners[0].mItems.data(), views.mOwners[0].mItems.capacity(),
                views.mOwners[1].mItems.data(), views.mOwners[1].mItems.capacity() };
        }

        size_t checkInventorySnapshot(DisposableTransferRehearsal& fixture, const RestoreContent& content)
        {
            using namespace Allocations;
            SerializedPair saved;
            saveFixture(fixture, content.mDeclarations, saved);
            SaveEnvelope envelope;
            envelope.mSourceOwner = fixture.mSourceOwner.getPtr().getCellRef().getRefNum();
            envelope.mDestinationOwner = fixture.mDestinationOwner.getPtr().getCellRef().getRefNum();
            const auto expected = savedViews(saved, envelope);
            const auto before = snapshot(fixture);
            auto sentinel = expected;
            sentinel.mOwners[0].mRevision = 1234567;
            auto output = sentinel;
            size_t allocations = 0;
            for (size_t failAt = 0; failAt <= allocations + 1; ++failAt)
            {
                output = sentinel;
                const auto storage = viewStorage(output);
                Trace trace;
                bool caught = false, correct = false;
                {
                    Observe observe(trace, failAt);
                    try
                    {
                        fixture.snapshotInventoryViews(output);
                        correct = output == expected;
                        InventoryViewSnapshot discarded;
                        output.swap(discarded); // Include complete output cleanup in tracking.
                    }
                    catch (const std::bad_alloc&)
                    {
                        caught = true;
                ...
                if (!failAt)
                    allocations = trace.mTotal;
                require(trace.mOutstanding == 0 && trace.mTrackingOverflow == 0
                        && trace.allocations(Phase::Validation) == 0 && trace.allocations(Phase::Publication) == 0
                        && snapshot(fixture) == before,
                    "snapshot allocated during validation/publication, leaked or mutated fixture");
                if (failAt && failAt <= allocations)
                    require(caught && trace.mFailures == 1 && output == sentinel && viewStorage(output) == storage,
                        "snapshot allocation failure published partial owner views");
                else
                    require(!caught && correct && trace.mFailures == 0,
                        "snapshot lost signed counts, dormant membership, selection or exact revision");
            }
            output = sentinel;
            const auto storage = viewStorage(output);
            const auto reject = [&] {
                ...
                require(caught && output == sentinel && viewStorage(output) == storage
                        && trace.allocations(Phase::Preparation) == 0 && trace.visits(Phase::Publication) == 0
                        && trace.mOutstanding == 0,
                    "invalid installed snapshot changed output or staged before validation");
            };
            auto& nodes = const_cast<PreparedContainerTransfer::MiscList&>(fixture.sourceStorage());
            if (!nodes.empty())
            {
                ...
                node.mRef.setRefNum({});
                reject();
                node.mRef.setRefNum(id);
                node.mWorldModel = nullptr;
                reject();
                node.mWorldModel = &fixture.mModel;
                const auto size = nodes.size();
                while (nodes.size() <= MaxTransferInventoryItems)
                {
                    nodes.push_back(node);
                    nodes.back().mWorldModel = nullptr;
                }
                reject();
                while (nodes.size() > size)
                    nodes.pop_back();
            }
            const auto selection = fixture.mSource.getSelectedEnchantItem();
            fixture.mSource.setSelectedEnchantItem(fixture.mDestination.end());
            reject();
            fixture.mSource.setSelectedEnchantItem(selection);
            fixture.mSource.setPtr(fixture.mDestinationOwner.getPtr(), fixture.mModel);
            reject();
            fixture.mSource.setPtr(fixture.mSourceOwner.getPtr(), fixture.mModel);
            require(snapshot(fixture) == before, "snapshot rejection checks did not restore fixture");
            // The same raw owner address/lifetime with foreign location metadata
            // is not an exact registry binding. Re-registration advances revision.
            auto foreignOwner = fixture.mSourceOwner.getPtr();
            foreignOwner.mContainerStore = &fixture.mDestination;
            fixture.mModel.registerPtr(foreignOwner);
            reject();
            fixture.mModel.registerPtr(fixture.mSourceOwner.getPtr());
            fixture.snapshotInventoryViews(output);
            require(output.mOwners[0].mRevision == fixture.mModel.getPtrRegistryRevision(),
                "snapshot reconstructed revision after owner re-registration");
            return allocations;
        }
        ...
        struct ContentionEvidence
        {
            ...
            size_t mReissued = 0;
            size_t mUncertainContenders = 0, mUncertainContinuations = 0;
            size_t mReissueSafe = 0, mReissueAllocations = 0, mReissueResultFailures = 0;
            unsigned mRemovalKinds = 0, mStackKinds = 0, mScriptKinds = 0, mSelectionKinds = 0;
        };

        struct CommandEvidence : ContentionEvidence
        {
        ...
        // after receipt, and in a receiver allocation; finally fail each of the
        // two owner-view allocations. All use actual committed outputs; only the
        // disposable fixture is recreated between commands.
        ...
        void checkNotificationConsumption(std::unique_ptr<const InventoryTransferSuccess>& output,
            ...
            InventoryViewSnapshot* views = nullptr, const InventoryViewSnapshot* acceptedViews = nullptr)
        {
            ...
            const auto beforeViews = views ? *views : InventoryViewSnapshot{};
            const auto beforeStorage = views ? viewStorage(*views) : viewStorage(beforeViews);
            struct Receiver final : InventoryNotificationConsumer
            {
                ...
                std::optional<InventoryViewConsumer> mViews;
                ...
                void receive(InventoryNotificationIntent intent) override
                {
                    ...
                    const bool fail = mMode && mMode <= 12 && mAttempts == (mMode - 1) % 4 + 1;
                    if (fail && mMode <= 4)
                        throw Failure{};
                    if (fail && mMode >= 9)
                    {
                    ...
                    if (mViews)
                        mViews->receive(intent);
                    mMailboxes[owner][mCounts[owner]++] = intent;
                    mOrder[mReceived++] = intent;
                    if (fail)
                        throw Failure{};
                }
            } receiver(output, expected, committed, mode, views);
            Allocations::Trace trace;
            InventoryNotificationDelivery result;
            const size_t failurePrefix = mode >= 13 ? (mode == 13 ? 1 : 3) : (mode ? (mode - 1) % 4 : 0);
            const size_t priorUpdates = views && mode >= 9 && mode <= 12 && failurePrefix >= 2 ? 1 : 0;
            {
                Allocations::Observe observe(trace, mode >= 13 ? mode - 12 : (mode >= 9 ? priorUpdates + 1 : 0));
            ...
            const size_t prefix = mode ? failurePrefix : total;
            const size_t received = prefix + (mode >= 5 && mode <= 8 ? 1 : 0);
            size_t updates = 0, index = 0;
            ...
            require(receiver.mValid && committed() && !output && result.mRevision == expected.mRevision
                    ...
                    && trace.mTotal == retained + (mode >= 9 ? 1u : 0u) && trace.mFailures == (mode >= 9 ? 1u : 0u)
                    && trace.mOutstanding == retained && trace.mTrackingOverflow == 0
            ...
            if (views)
            {
                require(acceptedViews, "missing independent accepted-save view oracle");
                for (size_t side = 0; side < 2; ++side)
                    require(views->mOwners[side]
                            == (updated[side] ? acceptedViews->mOwners[side] : beforeViews.mOwners[side]),
                        "independent owner view lost signed count, selection, revision or untouched owner state");
                if (!updated[0])
                    require(std::get<0>(viewStorage(*views)) == std::get<0>(beforeStorage),
                        "undelivered source view storage changed");
                if (!updated[1])
                    require(std::get<2>(viewStorage(*views)) == std::get<2>(beforeStorage),
                        "undelivered destination view storage changed");
                evidence.mViewAllocationFailures += mode >= 13;
            }
            size_t position = 0;
        ...
        void checkViewResynchronization(DisposableTransferRehearsal& fixture, InventoryViewSnapshot& views,
            const InventoryViewSnapshot& accepted, CommandEvidence& evidence)
        {
            const auto engineBefore = snapshot(fixture);
            const auto oldViews = views;
            const auto storage = viewStorage(views);
            // Both owner buffers must prepare before either view is published.
            // Failed recovery itself can be retried: only a read-only snapshot,
            // never a command or the already consumed notification batch.
            for (size_t ordinal : { 1u, 2u })
            {
                Allocations::Trace trace;
                bool caught = false;
                {
                    ...
                    try
                    {
                        fixture.snapshotInventoryViews(views);
                    }
                    catch (const std::bad_alloc&)
                    {
                        caught = true;
                ...
                require(caught && trace.mFailures == 1 && trace.mOutstanding == 0
                        && trace.visits(Allocations::Phase::Publication) == 0
                        && views == oldViews && viewStorage(views) == storage,
                    "failed resynchronization partially published owner views or leaked");
            }
            fixture.snapshotInventoryViews(views);
        ...
        InventoryTransferSuccess checkAlternatingCommand(DisposableTransferRehearsal& fixture, const SaveBindings& bindings,
            ...
            const InventoryTransferCommand* intended = nullptr, InventoryTransferCaller intendedCaller = {},
            ContentionEvidence* contention = nullptr, bool exhaustive = false)
        {
            using namespace Allocations;
            const auto owned = [](ESM::RefNum id) { return InventoryInstanceId{ id.mIndex, id.mContentFile }; };
            ...
            const InventoryTransferCaller caller{ intended ? intendedCaller.mInitiator
                : outward.mCommand.mInitiator == command.mSourceOwner ? command.mDestinationOwner : command.mSourceOwner };
            if (!intended)
                command.mInitiator = caller.mInitiator;
            require(command.mInitiator == caller.mInitiator, "continued intent does not match its independently trusted caller");
            ...
            InventoryTransferSuccess result;
            {
                ...
                require(result.mSourceCount
                            == item.getCellRef().getCount(false)
                                - (item.getCellRef().getCount(false) < 0 ? -1 : 1) * command.mQuantity
            ...
            if (contention)
            {
                const auto priorViews = views;
                const auto storage = viewStorage(views);
                const auto sentinelValue = *output;
                const auto reject = [&](FileFault fault, size_t ordinal = 0) {
                    faults = { fault, 17 };
                    Trace trace;
                    bool safe = false, failed = false;
                    {
                        Observe observe(trace, ordinal);
                        try { safe = !executeInventoryTransfer(fixture, caller, command, bindings, file, faults, output); }
                        catch (const std::bad_alloc&) { failed = true; }
                    }
                    require((ordinal ? failed && trace.mFailures == 1 && trace.mTotal == ordinal
                                           && faults.mReached == FileFault::None : safe && faults.mReached != FileFault::None)
                            && output.get() == sentinel && *output == sentinelValue && snapshot(fixture) == before
                            && views == priorViews && viewStorage(views) == storage && fileBytes(path) == priorBytes
                            && trace.mOutstanding == 0 && trace.mTrackingOverflow == 0
                            && trace.visits(Phase::Installation) == 0 && trace.visits(Phase::Publication) == 0
                            && !fixture.failedClosed() && !file.failedClosed(),
                        "reissued loser failure changed committed winner, views, output or persistence");
                    if (ordinal)
                    {
                        ++contention->mReissueAllocations;
                        contention->mReissueResultFailures += trace.mFailedPhase == Phase::Result;
                    }
                    else
                        ++contention->mReissueSafe;
                    return trace.mTotal;
                };
                const auto allocations = reject(FileFault::Create);
                ...
                if (exhaustive)
                    for (size_t ordinal = 1; ordinal <= allocations; ++ordinal)
                        reject(FileFault::Create, ordinal);
            }
            faults = {};
            Trace trace;
            {
        ...
        void checkContendedCommands(Install install, std::array<InventoryTransferCommand, 2> commands,
            const TransferSaveBytes& priorBytes, const SaveBindings& bindings, const std::filesystem::path& path,
            ContentionEvidence& evidence, bool exhaustive, bool opposing = false)
        {
            using namespace Allocations;
            ...
            auto temporary = path;
            temporary += ".tmp";
            const std::array uncertainFaults{ FileFault::ReplaceError, FileFault::AfterReplace, FileFault::Barrier,
                FileFault::ReadOpen, FileFault::ReadSize, FileFault::Read, FileFault::ReadEof, FileFault::ReadClose };
            for (size_t first : { 0u, 1u })
                for (size_t scenario = 0; scenario < (opposing ? 2 + uncertainFaults.size() : 2); ++scenario)
                {
                    const bool restart = scenario == 1;
                    ...
                    for (size_t index : { 0u, 1u })
                    {
                        ...
                        require(item.hasLiveReference() && item.getContainerStore() == &source
                                && item.getCellRef().getCount() >= command.mQuantity,
                            "contender did not name a live independently owned source item");
                        const auto removal = reverse
                            ? ContainerStoreRemoveContext{ fixture->mModel, fixture->mDestinationOwner.getPtr(),
                                *fixture->mDestinationAdd.mLocalScripts, fixture->mDestinationAdd.mInventoryUpdated }
                            : fixture->mRemoval;
                        ...
                        const std::array resolved{ ContainerStoreResolution(fixture->mOther, fixture->mOtherOwner.getPtr()) };
                        const auto pair = source.prepareTransfer(item, command.mQuantity, destination, removal, addition, resolved);
                        ...
                        const auto target = fixture->mModel.getPtr(pair.getDestinationIdentity());
                        const auto counter = fixture->mModel.getLastGeneratedRefNum();
                        const auto oldCount = target.isEmpty() ? 0 : target.getCellRef().getCount(false);
                        const auto sourceCount = item.getCellRef().getCount(false);
                        require(expected[index].mRevision == command.mExpectedRevision + 1
                                && expected[index].mSourceCount == sourceCount - (sourceCount < 0 ? -1 : 1) * command.mQuantity
                                && expected[index].mDestinationCount == oldCount + (oldCount < 0 ? -1 : 1) * command.mQuantity
                                && expected[index].mSourceSelection
                                    == (fixture->selectionIdentity(source) == engine(command.mItem)
                                            && expected[index].mSourceCount == 0 ? InventoryInstanceId{}
                                        : owned(fixture->selectionIdentity(source)))
                                && expected[index].mDestinationSelection == owned(fixture->selectionIdentity(destination))
                        ...
                        if (!item.getRefData().getLocals().getScriptId().empty())
                            require(pair.getDestinationItem().getRefData().getLocals().mShorts.front()
                                    == (callers[index].mInitiator == command.mDestinationOwner
                                            ? 1 : item.getRefData().getLocals().mShorts.front()),
                                "contender preparation substituted the other caller's script context");
                        evidence.mRemovalKinds |= expected[index].mSourceCount == 0 ? 2u : 1u;
                        evidence.mStackKinds |= target.isEmpty() ? 1u : 2u;
                        evidence.mScriptKinds |= item.getRefData().getLocals().getScriptId().empty() ? 1u : 2u;
                        const auto selection = fixture->selectionIdentity(source);
                        evidence.mSelectionKinds |= 1u << (!selection.isSet() ? 0
                            : selection == engine(command.mItem) ? 1
                            : fixture->mModel.getPtr(selection).getCellRef().getCount(false) == 0 ? 2 : 3);
                    }
                    ...
                    require(!opposing || (commands[0].mItem != commands[1].mItem
                                && commands[0].mQuantity != commands[1].mQuantity
                                && commands[0].mSourceOwner == commands[1].mDestinationOwner
                                && commands[0].mDestinationOwner == commands[1].mSourceOwner),
                        "opposing contenders lost independent items, unequal quantities or opposite directions");
                    ...
                    if (scenario >= 2)
                    {
                        const auto fault = uncertainFaults[scenario - 2];
                        const auto* firstStorage = outputs[first].get();
                        const auto firstValue = *outputs[first];
                        const auto priorViews = views;
                        const auto priorStorage = viewStorage(views);
                        const auto outputsPreserved = [&] {
                            return outputs[first].get() == firstStorage && *outputs[first] == firstValue
                                && outputs[last].get() == loserStorage && *outputs[last] == loserValue
                                && commands == commandsBefore;
                        };
                        faults = { fault, 17 };
                        Trace trace;
                        bool uncertain = false;
                        {
                            ...
                            try
                            {
                                executeInventoryTransfer(*fixture, callers[first], commands[first], bindings,
                                    file, faults, outputs[first]);
                            }
                            catch (const TestDurabilityUncertain&) { uncertain = true; }
                        }
                        const auto coherent = fileBytes(path);
                        require(uncertain && fixture->failedClosed() && file.failedClosed() && outputsPreserved()
                                && snapshot(*fixture) == before && views == priorViews && viewStorage(views) == priorStorage
                                && coherent == (fault == FileFault::ReplaceError ? priorBytes : accepted)
                                && trace.mOutstanding == 0 && trace.mTrackingOverflow == 0
                                && trace.allocations(Phase::Result) == 1 && trace.visits(Phase::Installation) == 0
                                && trace.visits(Phase::Publication) == 0 && !std::filesystem::exists(temporary),
                            "uncertain opposing contender installed, emitted success or lost coherent persistence");
                        TransferFileSink freshSink(path);
                        const auto blocked = [&](TransferFileSink& sink, size_t index) {
                            faults = {};
                            Trace closed;
                            bool caught = false;
                            {
                                ...
                                try
                                {
                                    executeInventoryTransfer(*fixture, callers[index], commands[index], bindings,
                                        sink, faults, outputs[index]);
                                }
                                catch (const TestDurabilityUncertain&) { caught = true; }
                            }
                            require(caught && closed.mTotal == 0 && faults.mReached == FileFault::None
                                    && faults.mWrites == 0 && faults.mReads == 0 && outputsPreserved()
                                    && fileBytes(path) == coherent && views == priorViews
                                    && viewStorage(views) == priorStorage,
                                "uncertainty let an opposing caller retry or changed its independent output/view");
                        };
                        for (auto* sink : { &file, &freshSink })
                            for (size_t index : { 0u, 1u })
                            {
                                blocked(*sink, index);
                                require(snapshot(*fixture) == before, "blocked opposing intent changed failed-closed fixture");
                            }
                        Trace closed;
                        bool snapshotBlocked = false;
                        {
                            Observe observe(closed, 1);
                            try { fixture->snapshotInventoryViews(views); }
                            catch (const TestDurabilityUncertain&) { snapshotBlocked = true; }
                        }
                        require(snapshotBlocked && closed.mTotal == 0 && views == priorViews
                                && viewStorage(views) == priorStorage,
                            "uncertain fixture published an unvalidated authoritative snapshot");
                        fixture.reset();
                        fixture = install(coherent);
                        const auto recoveredState = snapshot(*fixture);
                        for (size_t index : { 0u, 1u })
                            blocked(file, index); // A fresh fixture cannot clear the old sink's uncertainty.
                        SerializedPair recovered;
                        decodeTransferSave(coherent, bindings, recovered);
                        checkViewResynchronization(*fixture, views, savedViews(recovered, bindings.mEnvelope), flow);
                        require(snapshot(*fixture) == recoveredState && fileBytes(path) == coherent && outputsPreserved(),
                            "uncertainty recovery replayed a possibly persisted contender");
                        // Only the OTHER still-valid intended transfer is newly
                        // issued after validated restart; never replay the uncertain
                        // operation merely because no success was delivered.
                        const auto revision = fixture->mModel.getPtrRegistryRevision();
                        const auto next = checkAlternatingCommand(*fixture, bindings, freshSink, path, expected[first],
                            recovered, accepted, flow, views, true, &commands[last], callers[last]);
                        auto intended = commands[last];
                        intended.mExpectedRevision = revision;
                        require(next.mCommand == intended && next.mRevision == revision + 1 && outputsPreserved(),
                            "fresh uncertainty recovery lost the other contender's original intent");
                        const auto recoveredViews = views;
                        fixture.reset();
                        fixture = install(fileBytes(path), &views);
                        require(views == recoveredViews, "uncertainty continuation did not survive another version-4 restart");
                        ++evidence.mUncertainContenders;
                        ++evidence.mUncertainContinuations;
                        continue;
                    }
                    faults = {};
                    Trace trace;
                    ...
                    require(*outputs[first] == expected[first] && fileBytes(path) == accepted
                            ...
                            && trace.visits(Phase::Persistence) == 1
                            && faults.mWrites == (accepted.size() + faults.mChunk - 1) / faults.mChunk
                            && faults.mReads == faults.mWrites
                            && trace.mTrackingOverflow == 0,
                    ...
                    const auto next = checkAlternatingCommand(*fixture, bindings, file, path, expected[first],
                        saves[first], accepted, flow, views, true, opposing ? &commands[last] : nullptr, callers[last],
                        opposing && !restart ? &evidence : nullptr, exhaustive);
                    ...
                    {
                        auto intended = commands[last];
                        intended.mExpectedRevision = expected[first].mRevision;
                        require(next.mCommand == intended && commands == commandsBefore
                                && next.mCommand.mItem != expected[first].mCommand.mItem
                                && next.mCommand.mQuantity != expected[first].mCommand.mQuantity,
                            "resynchronized loser replaced its original intended item, direction or quantity");
                        ++evidence.mReissued;
                        fixture.reset();
                        fixture = install(fileBytes(path));
                        const auto finalViews = views;
                        fixture->snapshotInventoryViews(views);
                        require(views == finalViews, "fresh restart lost the reissued loser's accepted views");
                    }
                    ++evidence.mContended;
        ...
        void checkOpposingCoverage(const ContentionEvidence& evidence, size_t cases, unsigned selections = 15)
        {
            require(evidence.mContended == 4 * cases && evidence.mContentionStale == 12 * cases
                    && evidence.mContentionOrders == 3 && evidence.mContentionRestarts == 2 * cases
                    && evidence.mReissued == 4 * cases
                    ...
                    && evidence.mReissueSafe == 10 * cases && evidence.mReissueAllocations > 0
                    && evidence.mReissueResultFailures == 4
                    && evidence.mUncertainContenders == 16 * cases && evidence.mUncertainContinuations == 16 * cases,
                "opposing contention missed an arrival, delivery/restart or persistence/allocation failure boundary");
            require(evidence.mRemovalKinds == 3 && evidence.mStackKinds == 3 && evidence.mScriptKinds == 3
                    && evidence.mSelectionKinds == selections,
                "opposing contention missed partial/full removal, stacking, scripts or retained/dormant selections");
            std::cout << "Opposing contention: winners=" << evidence.mContended
                      << " stale=" << evidence.mContentionStale << " winner-restarts=" << evidence.mContentionRestarts
                      << " reissued=" << evidence.mReissued << " delivery-failures=" << evidence.mContentionDeliveryFailed
                      << " view-allocation-failures=" << evidence.mContentionViewFailures
                      << " safe-file-failures=" << evidence.mContentionSafe
                      << " allocation-failures=" << evidence.mContentionAllocations
                      << " reissue-safe-file-failures=" << evidence.mReissueSafe
                      << " reissue-allocation-failures=" << evidence.mReissueAllocations
                      << " uncertain=" << evidence.mUncertainContenders
                      << " fresh-continuations=" << evidence.mUncertainContinuations
                      << " selection-kinds=" << evidence.mSelectionKinds << '\n';
        }
            ...
            size_t allocationCount, FileFault fault, CommandEvidence& evidence)
        {
            ...
            InventoryTransferSuccess expectedResult;
            {
                ...
                expectedResult.mSourceSelection = ownedId(pair.getSourceSelection());
                expectedResult.mDestinationSelection = ownedId(pair.getDestinationSelection());
            }
            ...
            else
            {
                ...
                const auto acceptedViews = savedViews(expectedSave, envelope);
                checkNotificationConsumption(output, expectedResult,
                    ...
                    static_cast<unsigned>((evidence.mRepeated / 2 - 1) % 15), evidence, &views, &acceptedViews);
                checkViewResynchronization(fixture, views, acceptedViews, evidence);
    ...
    namespace
    {
        ...
        size_t checkRestartInstallation(Fresh makeFresh, const SerializedPair& decoded, const SaveBindings& bindings,
            const TransferSaveBytes& accepted, size_t& rejections)
        {
            using Fixture = DisposableTransferRehearsal;
            struct Inputs
            {
                std::unique_ptr<Fixture> mFixture;
                std::unique_ptr<const RestoredPair> mNodes;
                std::unique_ptr<const Fixture::RestartRegistry> mRegistry;
                std::unique_ptr<const Fixture::RestartScripts> mScripts;
                std::unique_ptr<const SerializedPair> mOutput
                    = std::make_unique<SerializedPair>(pairOutputSentinel<true>());
            };
            const auto prepare = [&](Inputs& input, const SerializedPair& saved) {
                const auto fresh = input.mFixture->restartScriptBindings();
                input.mFixture->prepareRestartRegistry(
                    saved, *input.mNodes, bindings.mEnvelope, fresh.mRegistry, input.mRegistry);
                input.mFixture->prepareRestartScripts(saved, *input.mNodes, bindings.mContent, bindings.mEnvelope,
                    *input.mRegistry, fresh, input.mScripts);
            };
            const auto make = [&](const SerializedPair& saved) {
                Inputs result;
                result.mFixture = makeFresh();
                restorePair(saved, bindings.mContent, result.mNodes);
                prepare(result, saved);
                return result;
            };
            const auto attempt = [&](Inputs& input, const SerializedPair& saved, const TransferSaveBytes& wire) {
                input.mFixture->installRestart(
                    wire, saved, bindings, input.mNodes, input.mRegistry, input.mScripts, input.mOutput);
            };
            const auto candidateState = [](const Inputs& input) {
                const auto storage = [](const auto& value) {
                    std::vector<std::tuple<const void*, ConstPtr, ReferenceLifetime::Witness, ESM::RefId>> entries;
                    for (const auto& entry : value.getEntries())
                        entries.emplace_back(
                            &entry, entry.getItem(), entry.getItem().getReferenceLifetime(), entry.getScript());
                    return std::tuple{ &value, value.getCursor(), entries };
                };
                return std::tuple{ input.mRegistry.get(), input.mRegistry->getBindings(), input.mScripts.get(),
                    storage(input.mScripts->getSourceStorage()), storage(input.mScripts->getDestinationStorage()) };
            };
            const auto success = [&](Inputs& input, const SerializedPair& saved, const TransferSaveBytes& wire,
                                     size_t failAt = SIZE_MAX) {
                auto& fixture = *input.mFixture;
                const auto registry = input.mRegistry->getBindings();
                const auto& sourceScripts = input.mScripts->getSourceStorage();
                const auto& destinationScripts = input.mScripts->getDestinationStorage();
                const auto addresses = [](const auto& storage) {
                    std::vector<const void*> result;
                    for (const auto& entry : storage.getEntries())
                        result.push_back(&entry);
                    return result;
                };
                const auto sourceAddresses = addresses(sourceScripts),
                           destinationAddresses = addresses(destinationScripts);
                const auto sourceCursor = sourceScripts.getCursor(), destinationCursor = destinationScripts.getCursor();
                ...
                if (failAt != SIZE_MAX)
                {
                    // Retained installation allocations belong to the fixture;
                    // measure its full destruction separately from allocating QA.
                    auto measured = make(saved);
                    {
                        Allocations::Observe observe(trace, failAt);
                        attempt(measured, saved, wire);
                        measured.mOutput.reset();
                        measured.mFixture.reset();
                    }
                    require(trace.mFailures == 0 && trace.mOutstanding == 0 && trace.mTrackingOverflow == 0
                            && trace.allocations(Allocations::Phase::Validation) == 0
                            && trace.allocations(Allocations::Phase::Revalidation) == 0
                            && trace.allocations(Allocations::Phase::Installation) == 0
                            && trace.allocations(Allocations::Phase::Retirement) == 0
                            && trace.allocations(Allocations::Phase::Publication) == 0
                            && trace.visits(Allocations::Phase::Installation) == 1
                            && trace.visits(Allocations::Phase::Publication) == 1,
                        "restart installation/publication allocated or leaked");
                }
                attempt(input, saved, wire);
                checkSavedValues(*input.mOutput, saved);
                require(!input.mNodes && !input.mRegistry && !input.mScripts
                        && fixture.mModel.snapshotPtrRegistry() == registry
                        && fixture.scriptNodes(fixture.mSourceScripts) == sourceAddresses
                        && fixture.scriptNodes(*fixture.mDestinationAdd.mLocalScripts) == destinationAddresses
                        && fixture.scriptCursor(fixture.mSourceScripts) == sourceCursor
                        && fixture.scriptCursor(*fixture.mDestinationAdd.mLocalScripts) == destinationCursor,
                ...
                for (const auto& [id, binding] : registry.mEntries)
                {
                    ...
                    require(item.hasLiveReference() && binding.references(item.mRef)
                            && item.mRef->mWorldModel == &fixture.mModel && item.getCellRef().getRefNum() == id
                            && item.mContainerStore == binding.getContainer(),
                        "restart lost installed node identity/ownership/lifetime");
                }
                require(nodeState(ConstPtr(&fixture.otherStorage().front())) == otherBefore
                        && fixture.cacheState(fixture.mOther) == otherCache && fixture.mNotifications == notifications
                        && listener.mCalls == 0,
                    "restart changed unrelated store or notified listeners");
                SerializedPair installed;
                saveFixture(fixture, bindings.mContent.mDeclarations, installed);
                ...
                std::unique_ptr<const RestoredPair> again;
                restorePair(decodedAgain, bindings.mContent, again);
                SerializedPair resaved;
                serializePair(*again, bindings.mContent.mDeclarations, resaved);
                checkSavedValues(resaved, saved);
                fixture.mSource.setContListener(nullptr);
                fixture.mDestination.setContListener(nullptr);
                return trace.mTotal;
            };
            const auto reject = [&](Inputs& input, auto call, size_t failAt = 0) {
                const auto fixtureBefore = snapshot(*input.mFixture);
                const auto nodesBefore = restoredState(input.mNodes);
                const auto outputBefore = pairOutputState(*input.mOutput);
                const auto* output = input.mOutput.get();
                const auto* registry = input.mRegistry.get();
                const auto* scripts = input.mScripts.get();
                const auto decodedBefore = pairOutputState(decoded);
                const auto bytesBefore = byteState(accepted);
                const auto contentBefore = contentState(bindings.mContent);
                Allocations::Trace trace;
                bool caught = false;
                {
                ...
                require(caught && trace.mOutstanding == 0 && trace.mTrackingOverflow == 0
                        ...
                        && trace.visits(Allocations::Phase::Publication) == 0
                        && snapshot(*input.mFixture) == fixtureBefore && restoredState(input.mNodes) == nodesBefore
                        && input.mOutput.get() == output && pairOutputState(*input.mOutput) == outputBefore
                        && input.mRegistry.get() == registry && input.mScripts.get() == scripts
                        && pairOutputState(decoded) == decodedBefore && byteState(accepted) == bytesBefore
                        && contentState(bindings.mContent) == contentBefore,
                    "restart rejection changed input, fixture or output, or leaked");
                ++rejections;
            };
            auto counted = make(decoded);
            const auto allocations = success(counted, decoded, accepted, 0);
            require(allocations > 0, "restart preallocation was not observed");
            for (size_t ordinal = 1; ordinal <= allocations; ++ordinal)
            {
                auto input = make(decoded);
                const auto candidates = candidateState(input);
                reject(input, [&] { attempt(input, decoded, accepted); }, ordinal);
                require(candidateState(input) == candidates, "allocation failure changed detached candidate storage");
                success(input, decoded, accepted); // Same retained candidates, healthy retry.
            }
            auto armed = make(decoded);
            success(armed, decoded, accepted, allocations + 1);
            ...
            for (int mode = 0; mode < 27; ++mode)
            {
                ...
                auto bad = decoded;
                auto wire = accepted;
                auto& inventory = nodes.mSource;
                const auto originalView = inventory.mViews.front();
                ...
                const auto& storage = input.mScripts->getSourceStorage();
                const auto* originalCursor = storage.getCursor();
                auto& entries = const_cast<LocalScripts::PreparedStorage::Entries&>(storage.getEntries());
                LocalScripts::PreparedStorage::Entries removed;
                std::unique_ptr<const RestoredPair> replacement;
                if (mode == 0)
                    ++nodes.mRestart.mRevision;
                if (mode == 1)
                    inventory.mViews.front() = {};
                if (mode == 2)
                    inventory.mViews.front().mContainerStore = &fixture.mSource;
                if (mode == 3)
                    inventory.mNodes.front().mBase
                        = bindings.mContent.mBases[originalBase == bindings.mContent.mBases[0]];
                if (mode == 4)
                    inventory.mNodes.front().mRef.setRefNum({ 789, -1 });
                if (mode == 5)
                    inventory.mNodes.front().mRef = originalRef.copyWithCount(123);
                if (mode == 6)
                    inventory.mNodes.front().mData.getLocals().mFloats.push_back(17.f);
                if (mode == 7)
                    inventory.mNodes.front().mData.mPhysicsPostponed = true;
                if (mode == 8)
                    fixture.mDestinationAdd.mLocalScripts = nullptr;
                if (mode == 9)
                    ++const_cast<PtrRegistry::Snapshot&>(input.mRegistry->getBindings()).mRevision;
                if (mode == 10)
                    const_cast<std::remove_const_t<std::remove_reference_t<decltype(storage.getCursor())>>&>(
                        storage.getCursor()) = originalCursor ? nullptr : &storage.getEntries().front();
                if (mode == 11)
                    removed.splice(removed.end(), entries, entries.begin());
                if (mode == 12)
                {
                    restorePair(decoded, bindings.mContent, replacement);
                    input.mNodes.swap(replacement);
                }
                if (mode == 13)
                    ++bad.mRestart.mRevision;
                if (mode == 14)
                    wire.back() ^= 1;
                if (mode == 15)
                    fixture.mSourceScripts.startIteration();
                if (mode == 16)
                    std::swap(inventory.mViews[0], inventory.mViews[1]);
                if (mode == 17)
                    fixture.mSource.setPtr(fixture.mOtherOwner.getPtr(), fixture.mModel);
                if (mode == 18)
                ...
                if (mode == 18)
                    --inventory.mProposedIdentities.front().mIndex;
                inventory.mViews.front() = originalView;
                inventory.mNodes.front().mRef = std::move(originalRef);
                ...
                const_cast<std::remove_const_t<std::remove_reference_t<decltype(storage.getCursor())>>&>(
                    storage.getCursor()) = originalCursor;
                if (mode == 11)
                    entries.splice(entries.begin(), removed);
                if (mode == 15)
                {
                    std::pair<ESM::RefId, Ptr> ignored;
                    while (fixture.mSourceScripts.getNext(ignored))
                ...
                success(input, decoded, accepted);
            }
            // Same-address service/store replacement invalidates old witnesses.
            for (bool service : { false, true })
            {
                ...
                if (service)
                {
                    std::destroy_at(&fixture.mSourceScripts);
                    std::construct_at(&fixture.mSourceScripts, fixture.mSourceAdd.mStore);
                }
                else
                {
                    std::destroy_at(&fixture.mSource);
                ...
                reject(input, [&] { attempt(input, decoded, accepted); });
                prepare(input, decoded);
                success(input, decoded, accepted);
            }
            for (int missing = 0; missing < 4; ++missing)
            {
                auto input = make(decoded);
                auto registry = std::move(input.mRegistry);
                auto scripts = std::move(input.mScripts);
                if (missing == 0)
                    input.mScripts = std::move(scripts);
                if (missing == 1)
                    input.mRegistry = std::move(registry);
                if (missing == 2)
                {
                    input.mRegistry = std::make_unique<Fixture::RestartRegistry>();
                    input.mScripts = std::move(scripts);
                }
                if (missing == 3)
                {
                    input.mRegistry = std::move(registry);
                    input.mScripts = std::make_unique<Fixture::RestartScripts>();
                }
                reject(input, [&] { attempt(input, decoded, accepted); });
                if (registry)
                    input.mRegistry = std::move(registry);
            ...
            {
                auto input = make(decoded);
                // Reconstruct both nodes and fixture while keeping stale detached
                // candidates. Equal values/addresses cannot establish ownership.
                input.mFixture.reset();
                input.mFixture = makeFresh();
                reject(input, [&] { attempt(input, decoded, accepted); });
                prepare(input, decoded);
                auto& nodes = const_cast<RestoredPair&>(*input.mNodes).mSource.mNodes;
                nodes.pop_front();
                reject(input, [&] { attempt(input, decoded, accepted); });
                restorePair(decoded, bindings.mContent, input.mNodes);
                reject(input, [&] { attempt(input, decoded, accepted); });
                prepare(input, decoded);
                success(input, decoded, accepted);
            }
            {
                auto input = make(decoded);
                auto& fixture = *input.mFixture;
                ManualRef itemTemplate(fixture.mSourceAdd.mStore, bindings.mContent.mBases[0]->mId);
                const auto item = *fixture.mSource.add(itemTemplate.getPtr(), 2, fixture.mSourceAdd);
                const std::array resolved{ ContainerStoreResolution(fixture.mOther, fixture.mOtherOwner.getPtr()) };
                auto pair = fixture.mSource.prepareTransfer(
                    item, 1, fixture.mDestination, fixture.mRemoval, fixture.mDestinationAdd, resolved);
                bool uncertain = false;
                try
                {
                    fixture.commitDurably(std::move(pair), bindings.mContent.mDeclarations,
                        [](const SerializedPair&) { return TestPersistenceResult::Uncertain; });
                }
                catch (const TestDurabilityUncertain&)
                {
                    uncertain = true;
                }
                require(uncertain && fixture.failedClosed(), "restart test did not reach sticky uncertainty");
                for (int retry = 0; retry < 2; ++retry)
                    reject(input, [&] {
                        // Require the sticky signal, not a generic stale-fixture rejection.
                        bool blocked = false;
                        try
                        {
                            attempt(input, decoded, accepted);
                        }
                        catch (const TestDurabilityUncertain&)
                        {
                            blocked = true;
                        }
                        require(blocked && fixture.failedClosed(), "restart bypassed sticky uncertainty");
                        throw TestDurabilityUncertain{};
                    });
                input.mFixture.reset(); // Uncertain fixture is discarded, never repaired.
                input.mFixture = makeFresh();
                prepare(input, decoded);
                success(input, decoded, accepted);
            }
            // Preserve distinct empty services, reversed registration order,
            // exhausted saved counters and even completely empty inventories.
            for (int empty = 0; empty < 5; ++empty)
            {
                ...
                {
                    auto& service = saved.mScripts.mServices[side];
                    if (empty == 4 || (empty & (1 << side)))
                        service.mEntries.clear();
                    else
                        std::reverse(service.mEntries.begin(), service.mEntries.end());
                    service.mCursor = service.mEntries.size();
                }
                if (empty == 4)
                {
                ...
                encodeTransferSave(saved, bindings, wire);
                auto input = make(saved);
                success(input, saved, wire);
                // Installed empty fixtures must not permit repeated installation.
                restorePair(saved, bindings.mContent, input.mNodes);
                bool caught = false;
                try
                {
                    prepare(input, saved);
                ...
                require(caught, "installed restart fixture accepted another preparation");
                ++rejections;
            }
            return allocations;
        }
            ...
            bool exhaustive, CommandEvidence& evidence, const InventoryViewSnapshot& retainedViews, bool reverse)
        {
            ...
            const auto installed = [&](const TransferSaveBytes& bytes, InventoryViewSnapshot* receiving = nullptr) {
                ...
                InventoryViewSnapshot bootstrap;
                fixture->snapshotInventoryViews(bootstrap);
                require(bootstrap == savedViews(decoded, bindings.mEnvelope),
                    "fresh restart snapshot differs from accepted version-4 owner views");
                if (receiving)
            ...
            const auto run = [&](const SerializedPair& prior, const TransferSaveBytes& priorBytes, size_t failAt,
                ...
                auto views = retainedViews; // Outlived the original fixture and its delivered success.
                ...
                if (fault != FileFault::None)
                {
                    ...
                    if (quantity == 1)
                    {
                        ...
                        const auto acceptedViews = savedViews(actual, bindings.mEnvelope);
                        ...
                        checkNotificationConsumption(output, recoveredResult,
                            ...
                            0, evidence, &views, &acceptedViews);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        checkNotificationConsumption(output, expectedResult,
                            ...
                            listenerMask == 3 ? static_cast<unsigned>((evidence.mRepeated - 1) % 15) : 0, evidence,
                            &views, &acceptedViews);
                        checkViewResynchronization(*fixture, views, acceptedViews, evidence);
                        require(snapshot(*fixture) == committed && fileBytes(path) == expectedBytes,
                            "post-restart delivery replayed mutation or changed committed persistence");
                        fixture.reset();
                        const auto beforeRestartViews = views;
                        auto second = installed(fileBytes(path), &views);
                        require(views == beforeRestartViews,
                            "fresh restart changed retained owned views at the same accepted revision");
                        require(
            ...
            checkContendedCommands(installed, { commandFor(saved), commandFor(saved) },
                accepted, bindings, path, evidence, exhaustive);
            // A separate synthetic version-4 seed supplies two live contenders,
            // even when the preceding full-removal fixture left a dormant node.
            // Change only their signed counts; retain identities, selections,
            // script/service metadata and exact saved counters before installation.
            auto opposingSaved = saved;
            auto& firstInventory = reverse ? opposingSaved.mDestination : opposingSaved.mSource;
            auto& secondInventory = reverse ? opposingSaved.mSource : opposingSaved.mDestination;
            const auto firstPosition = std::find(firstInventory.mProposedIdentities.begin(),
                firstInventory.mProposedIdentities.end(), sourceId) - firstInventory.mProposedIdentities.begin();
            auto& firstObject = firstInventory.mObjects.at(firstPosition);
            const auto secondObject = std::find_if(secondInventory.mObjects.begin(), secondInventory.mObjects.end(),
                [&](const auto& object) { return object.mRef.mRefID == firstObject.mRef.mRefID; });
            require(secondObject != secondInventory.mObjects.end(), "opposing seed lacks the second source item");
            const auto secondPosition = secondObject - secondInventory.mObjects.begin();
            firstObject.mRef.mCount = firstObject.mRef.mCount < 0 ? -3 : 3;
            secondObject->mRef.mCount = secondObject->mRef.mCount < 0 ? -5 : 5;
            std::array opposingCommands{ commandFor(opposingSaved), commandFor(opposingSaved) };
            std::swap(opposingCommands[1].mSourceOwner, opposingCommands[1].mDestinationOwner);
            opposingCommands[1].mItem = owned(secondInventory.mProposedIdentities.at(secondPosition));
            opposingCommands[0].mQuantity = quantity == 1 ? 1 : 3;
            opposingCommands[1].mQuantity = quantity == 1 ? 2 : 5;
            TransferSaveBytes opposingBytes;
            encodeTransferSave(opposingSaved, bindings, opposingBytes);
            checkContendedCommands(installed, opposingCommands, opposingBytes, bindings, path,
                evidence.mOpposing, exhaustive, true);
            unchangedInputs();
            ...
            bool exhaustive = false, bool returnTransfers = false)
        {
            ...
            InventoryViewSnapshot initialViews;
            {
                const auto prepared = make();
                ...
                if (continuation)
                {
                    ...
                    initialResult.mSourceSelection = owned(prepared.getSourceSelection());
                    initialResult.mDestinationSelection = owned(prepared.getDestinationSelection());
                }
            }
            TransferSaveBytes bytes;
            encodeTransferSave(saved, bindings, bytes);
            ...
            if (continuation)
            {
                ...
                require(original->commit(make(), content.mDeclarations,
                            [&](const SerializedPair& committed) {
                                ...
                                require(actual == bytes, "restart input differs from synchronously accepted save");
                                return true;
                            }),
                    "restart original save was not accepted");
            original.reset();
        ...
        const ESMStore& content, AllocationCheck allocationCheck, const std::filesystem::path& scratch = {})
    {
        ...
        if (allocationFailures)
        {
            ...
            if (command || restartCommand)
            {
                ...
                if (restartCommand)
                    // Forward seeds select the contender itself or a dormant
                    // other node. Return/selection matrices also retain a live
                    // selection different from the independently transferred item.
                    checkOpposingCoverage(commandEvidence.mOpposing, cases, returnTransfers ? 15u : 7u);
                ...
                std::cout << (restartCommand ? "Post-restart inventory command: cases=" : "Inventory command: cases=")
                          ...
                          << " view-resynchronizations=" << commandEvidence.mViewResyncs
                          << " view-receiver-allocation-failures=" << commandEvidence.mViewAllocationFailures
            ...
            if (restartInstallation)
            {
                require(localRejections > 0 && totals.mTotal > 0, "restart installation coverage missing");
                std::cout << "Fresh restart installation: cases=" << cases << " individually-failed=" << totals.mTotal
                          << " rejected-attempts=" << localRejections
                          << " validation=0 installation=0 retirement=0 publication=0 remaining-after-cleanup=0\n";
                return;
            }
            if (restartScripts)
            {
        ...
        const ESMStore& content, const std::filesystem::path& scratch, bool snapshotsOnly, bool returnTransfers)
    {
        ...
        size_t cases = 0, rejected = 0, snapshotAllocations = 0;
        for (bool shared : { false, true })
            for (bool scripted : { false, true })
                for (int quantity : { 1, 3 })
                    for (int selection : { 0, 1, 2, 3 })
                    {
                        ...
                        if (snapshotsOnly)
                            snapshotAllocations += checkInventorySnapshot(*fixture, suppliedContent);
                        ...
                        {
                            if (quantity == 3)
                                item->getCellRef() = item->getCellRef().copyWithCount(-4);
                            snapshotAllocations += checkInventorySnapshot(*fixture, suppliedContent);
                            InventoryViewSnapshot surviving;
                            fixture->snapshotInventoryViews(surviving);
                            const auto expected = surviving;
                            fixture.reset();
                            require(surviving == expected && scripts.mRuns == 0,
                                "snapshot retained a borrowed engine value or executed script instructions");
                            ++cases;
                            continue;
                        }
        ...
        if (snapshotsOnly)
        {
            require(cases == 32 && snapshotAllocations == 64, "snapshot matrix allocation coverage missing");
            std::cout << "Inventory view snapshots: cases=" << cases << " empty=32 allocation-failures="
        ...
        checkOpposingCoverage(evidence.mOpposing, cases);
        require(std::filesystem::remove(scratch / "inventory.bin") && std::filesystem::is_empty(scratch),
    ...
    {
        checkTransferRehearsalCases(content, AllocationCheck::RestartInstallation);
    }
