...
namespace TES3MP::Native::Testing
{
        ...
        bool strike, bool knockout, bool defense, bool shield, bool effectLifecycle, bool constantEffects, bool generalConstants, bool durableCasters, bool actorCasts, bool automaticCasts)
    {
        ...
        // inventory mechanics come from OpenMW; this is not a published-mod proof.
        {
            ...
            std::array<ESM::Enchantment, 4> passiveEnchantments;
            std::array<ESM::Clothing, 4> passiveShirts;
            if (constantEffects)
            {
                for (int i = 0; i < 2; ++i)
                {
                    ...
                    if (generalConstants)
                        enchantment.mEffects.populate({
                            {ESM::MagicEffect::FortifyAttribute, {}, ESM::Attribute::Luck, ESM::RT_Self, 0, 0, i ? 11 : 7, i ? 11 : 7},
                            {ESM::MagicEffect::FortifyAttribute, {}, ESM::Attribute::Strength, ESM::RT_Self, 0, 0, 0, 15},
                            {ESM::MagicEffect::FortifySkill, ESM::Skill::Axe, {}, ESM::RT_Self, 0, 0, 4, 9},
                            {ESM::MagicEffect::FortifyAttribute, {}, ESM::Attribute::Strength, ESM::RT_Self, 0, 0, 1, 3},
                            {ESM::MagicEffect::FortifyAttribute, {}, ESM::Attribute::Intelligence, ESM::RT_Self, 0, 0, 9, 9}});
                ...
                if (generalConstants)
                {
                    const auto& settings = base.store().get<ESM::GameSetting>();
                    passiveMagickaMultiplier = settings.find("fNPCbaseMagickaMult")->mValue.getFloat();
                    passiveSettings.damageStrengthBase = settings.find("fDamageStrengthBase")->mValue.getFloat();
                    passiveSettings.damageStrengthMultiplier = settings.find("fDamageStrengthMult")->mValue.getFloat();
                    passiveSettings.unarmoredBase1 = settings.find("fUnarmoredBase1")->mValue.getFloat();
                    passiveSettings.unarmoredBase2 = settings.find("fUnarmoredBase2")->mValue.getFloat();
                    passiveSettings.combatArmorMinimumMultiplier = settings.find("fCombatArmorMinMult")->mValue.getFloat();
                    defenseWeapon = *base.store().get<ESM::Weapon>().find(ESM::RefId::stringRefId("iron shortsword"));
                    auto& enchantment = passiveEnchantments[2]; enchantment.blank();
                    enchantment.mId = ESM::RefId::stringRefId("npc_passive_ring_enchantment");
                    enchantment.mData.mType = ESM::Enchantment::ConstantEffect;
                    enchantment.mEffects.populate({
                        {ESM::MagicEffect::ResistFire, {}, {}, ESM::RT_Self, 0, 0, 1, 13},
                        {ESM::MagicEffect::FortifyAttribute, {}, ESM::Attribute::Agility, ESM::RT_Self, 0, 0, 0, 0}});
                    auto& ring = passiveShirts[2]; ring = passiveShirts[0];
                    ring.mId = ESM::RefId::stringRefId("npc_passive_ring");
                    ring.mData.mType = ESM::Clothing::Ring; ring.mParts.mParts.clear();
                    ring.mEnchant = enchantment.mId;
                    npc.mInventory.mList.push_back({1, ring.mId});
                    auto& unsupported = passiveEnchantments[3]; unsupported = passiveEnchantments[0];
                    unsupported.mId = ESM::RefId::stringRefId("npc_passive_unsupported");
                    unsupported.mEffects.mList.back().mData.mEffectID = ESM::MagicEffect::RestoreHealth;
                    unsupported.mEffects.mList.back().mData.mAttribute = {};
                    auto& invalidShirt = passiveShirts[3]; invalidShirt = passiveShirts[0];
                    invalidShirt.mId = ESM::RefId::stringRefId("npc_passive_invalid_shirt");
                    invalidShirt.mData.mValue = 1; invalidShirt.mEnchant = unsupported.mId;
                    npc.mInventory.mList.push_back({1, invalidShirt.mId});
                }
            }
            ...
            if (strike)
            {
                ...
                if (durableCasters)
                    strikeEnchantment.mEffects.populate({
                        {ESM::MagicEffect::FireDamage, {}, {}, ESM::RT_Self, 0, 2, 1000, 1000},
                        {ESM::MagicEffect::ResistNormalWeapons, {}, {}, ESM::RT_Target, 0, 20, 60, 60}});
            ...
            if (spell)
            {
                ...
                if (projectile)
                {
                    ...
                    if (collection)
                    {
                        ...
                        onceEnchantment.mId = ESM::RefId::stringRefId("npc_once_damage");
                        onceEnchantment.mData.mType = ESM::Enchantment::CastOnce;
                        onceItem = usedItem;
            ...
            if (automaticCasts)
            {
                // A higher-rated but currently unexecutable Touch source must
                // not hide the Target spell that the shared launch can execute.
                targetDamage.mEffects.populate({
            ...
            if (spell)
            {
                ...
                if (projectile)
                {
                    ...
                    {
                        out.startRecord(ESM::Enchantment::sRecordId, 0);
                        onceEnchantment.save(out); out.endRecord(ESM::Enchantment::sRecordId);
                        out.startRecord(ESM::Clothing::sRecordId, 0);
                        onceItem.save(out); out.endRecord(ESM::Clothing::sRecordId);
                    }
                }
            }
        ...
        }
        {
            ...
            if (effectLifecycle && strike)
            {
                ...
                if (automaticCasts)
                {
                    ...
                    for (size_t i = 0; i < entities.size(); ++i)
                    {
                        ...
                        entities[i] = std::get<CanonicalPlayerEntityState>(advanceCanonicalSpatialState(old,
                            id<ServerTick>(1), Transform(old.transform().cell(),
                                Position3((i ? -300 : 60)*1024, -400*1024, 1024), old.transform().orientation()),
                    ...
                    const auto sessions = authority.activeSessions();
                    const std::vector<CanonicalSessionProgress> savedSessions(sessions.begin(), sessions.end());
                    size_t launches = 0, contacts = 0;
                    bool firstTarget = false, secondTarget = false, suppressedActive = false;
                    for (uint64_t tick = 1; tick <= 100; ++tick)
                    {
                        if (tick == 24 || tick == 56 || tick == 72)
                        {
                            auto activeSessions = savedSessions;
                            if (tick == 24) activeSessions.erase(activeSessions.begin());
                            if (tick == 56) activeSessions.clear();
                            authority = std::get<CanonicalServerState>(createCanonicalServerState(entities, activeSessions));
                            service.synchronizeCells(authority);
                            if (restored) restored->synchronizeCells(authority);
                        }
                        const auto preparePlayer = [&](InventoryService& owner) -> std::unique_ptr<PreparedNativeInventory>
                        {
                            if (tick != 8) return {};
                            ClientMagicUseCommand use{id<SessionId>(1), SessionGeneration::initial(),
                                CommandSequence::initial(), id<CommandId>(1), id<CanonicalRevision>(1),
                                MagicUseSourceKind::Spell, source("npc_elemental_damage"), MagicUseTargetKind::Actor, npc.mIdentity,
                            ...
                            auto playerCast = owner.prepareMagicUse(authority, proposal, id<ServerTick>(tick));
                            require(bool(playerCast), "Automatic NPC scheduling blocked concurrent player preparation");
                            return playerCast;
                        };
                        const auto before = bytes(service);
                        auto step = service.prepareNativeTick(authority, id<ServerTick>(tick), 1.f/30, preparePlayer(service));
                        require(bool(step), "Automatic NPC tick missing");
                        const auto event = service.projectCombatEvents(authority, id<SessionId>(tick >= 24 && tick < 56 ? 2 : 1),
                            id<ServerTick>(tick), id<CanonicalRevision>(tick), step.get());
                        if (tick == 8)
                        {
                            const auto other = service.projectCombatEvents(authority, id<SessionId>(2),
                                id<ServerTick>(tick), id<CanonicalRevision>(tick), step.get());
                            require(event && other && event->magicEvents().size() == 2
                                && std::ranges::equal(event->magicEvents(), other->magicEvents())
                                && !event->magicEvents()[0].actorCaster() && event->magicEvents()[1].actorCaster(),
                                "Automatic and player launches did not reach both observer projections");
                        }
                        ...
                        require(step->commit([&](auto image) { rejected.assign(image.begin(), image.end());
                            ...
                            && bytes(service) == before, "Rejected autonomous tick leaked RNG, payment, effects or state");
                        std::unique_ptr<PreparedNativeInventory> replay;
                        if (restored) replay = restored->prepareNativeTick(authority, id<ServerTick>(tick), 1.f/30, preparePlayer(*restored));
                        // Re-preparing the same autonomous decision after rejection must
                        // produce the identical image, not just retry the old candidate.
                        {
                            auto retry = service.prepareNativeTick(authority, id<ServerTick>(tick), 1.f/30, preparePlayer(service));
                            require(retry && retry->commit([&](auto image) {
                                require(std::ranges::equal(image, rejected), "Rejected autonomous retry changed its decision or RNG");
                                return CanonicalDurabilityResult::Rejected;
                            }) == CanonicalDurabilityResult::Rejected, "Autonomous retry unexpectedly committed");
                        }
                        require(step->commit(accepted) == CanonicalDurabilityResult::Committed, "Autonomous tick commit failed");
                        if (restored)
                            require(replay && replay->commit(accepted) == CanonicalDurabilityResult::Committed
                                && bytes(service) == bytes(*restored), "Autonomous restart diverged from uninterrupted simulation");
                        const auto image = bytes(service);
                        const auto state = readActorCampaign({reinterpret_cast<const char*>(image.data()), image.size()});
                        size_t newFlights = 0;
                        for (const auto& flight : state.projectiles)
                            if (flight.casterKind == 2 && flight.commandId == tick)
                            {
                                ++launches; ++newFlights;
                                firstTarget |= flight.target == 1;
                                secondTarget |= flight.target == 2;
                                require(tick % 8 == 0 && (tick < 24 || tick >= 72 || flight.target == 2),
                                    "Autonomous cadence or disconnected-target eligibility changed");
                            }
                        require(newFlights <= 1 && (tick >= 8 || state.projectiles.empty()), "Automatic scheduling duplicated a flight");
                        if (tick >= 56 && tick < 72) require(!newFlights, "Offline NPC scheduled a cast");
                        if (tick == 8)
                        {
                        ...
                        if (tick == 7 || tick == 8)
                        {
                            ...
                            require(bytes(*restored) == image, "Automatic admission/in-flight restart altered state");
                        }
                        if (tick == 16) require(!newFlights, "Automatic AI stacked an in-flight source");
                        if (tick == 48)
                        {
                            suppressedActive = !newFlights && std::ranges::any_of(state.timedEffects,
                                [](const auto& effect) { return effect.actor == 1 && effect.casterKind == 2; });
                        }
                    }
                    require(firstTarget && secondTarget && launches >= 2 && contacts > launches && suppressedActive,
                        "Autonomous two-target launch/contact or active-source suppression missing");
                    std::cout << "automatic spells: shared player/NPC launches; two targets/observers; disconnect/offline/rejoin; "
                        "in-flight restart exact; rejected decisions/effects atomic; source activity suppresses repeats\n";
                    return;
                }
                if (actorCasts)
                {
                ...
                {
                    auto entities = std::vector(authority.players().begin(), authority.players().end());
                    const auto& origin = entities.front();
                    entities.front() = std::get<CanonicalPlayerEntityState>(advanceCanonicalSpatialState(
                        origin, id<ServerTick>(1), Transform(origin.transform().cell(),
                            Position3(60*1024, -200*1024, 1024), origin.transform().orientation()), LinearVelocity3(0, 0, 0)));
                    auto flightAuthority = std::get<CanonicalServerState>(createCanonicalServerState(
                        entities, authority.activeSessions()));
                    InventoryService flight(loadout.store(), loadout.readers(), bindCaster());
                    flight.synchronizeCells(flightAuthority);
                    uint64_t sourceId = 14695981039346656037ull;
                    for (unsigned char c : std::string_view("npc_target_resistance"))
                        sourceId = (sourceId ^ c) * 1099511628211ull;
                    ClientMagicUseCommand use{id<SessionId>(1), SessionGeneration::initial(),
                        CommandSequence::initial(), id<CommandId>(1), id<CanonicalRevision>(1),
                        MagicUseSourceKind::Spell, sourceId, MagicUseTargetKind::Actor, npc.mIdentity,
                        id<ServerTick>(1), CombatRevision::initial(), CombatRevision::initial(),
                        InventoryRevision::initial()};
                    const auto* player = flightAuthority.findPlayer(id<PlayerId>(1));
                    ServerCommandProposal proposal(id<SessionId>(1), SessionGeneration::initial(),
                        use.commandSequence, use.commandId, use.observedCanonicalRevision,
                        EntityPrecondition(player->entityId(), player->entityRevision(), player->authorityEpoch()),
                        MagicUseCommandProposal(use));
                    const auto before = bytes(flight);
                    auto cast = flight.prepareMagicUse(flightAuthority, proposal, id<ServerTick>(1));
                    require(bool(cast), "Durable player caster launch rejected");
                    auto launch = flight.prepareNativeTick(flightAuthority, id<ServerTick>(1), 1.f/30, std::move(cast));
                    require(launch && launch->commit([](auto) { return CanonicalDurabilityResult::Rejected; })
                        == CanonicalDurabilityResult::Rejected && bytes(flight) == before,
                        "Rejected durable caster launch leaked cost or projectile");
                    require(launch->commit(accepted) == CanonicalDurabilityResult::Committed,
                        "Durable player caster launch did not commit");
                    const auto checkpoint = bytes(flight);
                    const auto state = readActorCampaign({reinterpret_cast<const char*>(checkpoint.data()), checkpoint.size()});
                    require(state.projectiles.size() == 1 && state.projectiles[0].caster == 1
                        && state.projectiles[0].casterKind == 1 && state.projectiles[0].casterLife == 1,
                        "Projectile omitted durable player caster identity");
                    std::vector<ESM::RefId> refs{npc.mRef.mRefID, chest.mRef.mRefID};
                    for (const auto& [id, record] : MWWorld::inventoryRecords(content)) refs.push_back(record);
                    for (const auto& [id, record] : MWWorld::inventorySoulRecords(content)) refs.push_back(record);
                    InventoryService restored(loadout.store(), loadout.readers(), bindCaster(), true);
                    restored.recover(checkpoint, refs); restored.synchronizeCells(flightAuthority);
                    require(bytes(restored) == checkpoint, "Projectile caster restart changed committed bytes");
                    require(state.timedEffects.empty(), "Unexpected initial projectile fixture effects");
                    const auto projectileOffset = size_t(state.inventory.data()
                        - reinterpret_cast<const char*>(checkpoint.data())) - 8 - 136;
                    for (const auto [offset, value] : std::array<std::pair<size_t, uint64_t>, 4>{
                        {{120, 0}, {120, 2}, {128, 0}, {128, 2}}})
                    {
                        auto invalid = checkpoint;
                        for (size_t i = 0; i < 8; ++i) invalid.at(projectileOffset + offset + i) = std::byte(value >> (8*i));
                        bool rejected = false;
                        try { restored.recover(invalid, refs); } catch (const std::invalid_argument&) { rejected = true; }
                        require(rejected && bytes(restored) == checkpoint,
                            "Malformed projectile caster accepted or leaked recovery");
                    }
                    const auto reconnected = players(SessionGeneration::initial().next().value(), 1, 2);
                    flightAuthority = std::get<CanonicalServerState>(createCanonicalServerState(
                        entities, reconnected.activeSessions()));
                    flight.synchronizeCells(flightAuthority); restored.synchronizeCells(flightAuthority);
                    bool contacted = false;
                    for (uint64_t tick = 2; tick <= 12 && !contacted; ++tick)
                    {
                        auto step = flight.prepareNativeTick(flightAuthority, id<ServerTick>(tick), 1.f/30, {});
                        auto replay = restored.prepareNativeTick(flightAuthority, id<ServerTick>(tick), 1.f/30, {});
                        require(step && replay && step->commit(accepted) == CanonicalDurabilityResult::Committed
                            && replay->commit(accepted) == CanonicalDurabilityResult::Committed
                            && bytes(flight) == bytes(restored), "Projectile caster replay diverged");
                        const auto image = bytes(flight);
                        const auto current = readActorCampaign({reinterpret_cast<const char*>(image.data()), image.size()});
                        contacted = !current.timedEffects.empty();
                        if (contacted)
                            require(current.timedEffects.size() == 1 && current.timedEffects[0].caster == 1
                                && current.timedEffects[0].casterKind == 1 && current.timedEffects[0].casterLife == 1,
                                "Projectile contact lost player caster identity");
                    }
                    require(contacted, "Durable player projectile never installed its timed effect");
                }
                bool hit = false;
                for (uint64_t tick = 1; tick <= 64 && !hit; ++tick)
                {
                    ...
                    if (!hit) continue;
                    require(state.timedEffects.size() == (durableCasters ? 2 : 1) && state.timedEffects.front().actor == 2
                    ...
                    if (durableCasters)
                        for (const auto& effect : state.timedEffects)
                            require(effect.casterKind == 2 && effect.casterLife == 1,
                                "NPC strike lost caster kind or launch life");
                    InventoryService restored(loadout.store(), loadout.readers(), bindCaster(), true);
                    std::vector<ESM::RefId> refs{npc.mRef.mRefID, chest.mRef.mRefID};
                    for (const auto& [id, record] : MWWorld::inventoryRecords(content)) refs.push_back(record);
                    for (const auto& [id, record] : MWWorld::inventorySoulRecords(content)) refs.push_back(record);
                ...
                if (durableCasters)
                {
                    ...
                    for (const auto& [id, record] : MWWorld::inventoryRecords(content)) refs.push_back(record);
                    for (const auto& [id, record] : MWWorld::inventorySoulRecords(content)) refs.push_back(record);
                    const auto decoded = [](const auto& service) {
                        const auto image = service.inventoryImage();
                        return readActorCampaign({reinterpret_cast<const char*>(image.data()), image.size()});
                    };
                    auto state = decoded(service);
                    const auto lastTick = state.tick + 60;
                    while (state.life->generation == 1 && state.tick < lastTick)
                    {
                        const auto before = bytes(service);
                        auto pending = service.prepareNativeTick(authority, id<ServerTick>(state.tick + 1), 1.f/30, {});
                        require(pending && pending->commit([](auto) { return CanonicalDurabilityResult::Rejected; })
                                == CanonicalDurabilityResult::Rejected && bytes(service) == before,
                            "Rejected caster death/respawn leaked effects or attribution");
                        require(pending->commit(accepted) == CanonicalDurabilityResult::Committed,
                            "Caster death/respawn failed to commit");
                        state = decoded(service);
                    }
                    require(state.life->generation == 2 && state.life->deaths.size() == 1
                        && state.life->deaths[0].killer == npc.mIdentity
                        && state.life->deaths[0].killerKind == 2 && state.life->deaths[0].killerLife == 1,
                        "Timed self death lost original NPC placement/life");
                    require(state.timedEffects.size() == 1 && state.timedEffects[0].actor < 2
                        && state.timedEffects[0].casterKind == 2 && state.timedEffects[0].casterLife == 1,
                        "Respawn reassigned or discarded an old-life effect on another actor");
                    const auto checkpoint = bytes(service);
                    InventoryService restored(loadout.store(), loadout.readers(), bindCaster(), true);
                    restored.recover(checkpoint, refs); restored.synchronizeCells(authority);
                    require(bytes(restored) == checkpoint, "Old-life caster restart changed committed bytes");
                    const auto replaceWord = [](auto& image, size_t offset, uint64_t value) {
                        for (size_t i = 0; i < 8; ++i) image.at(offset+i) = std::byte(value >> (8*i));
                    };
                    const auto effectOffset = size_t(state.inventory.data()
                        - reinterpret_cast<const char*>(service.inventoryImage().data())) - 112;
                    const auto rejectRecovery = [&](auto invalid) {
                        bool rejected = false;
                        try { restored.recover(invalid, refs); } catch (const std::invalid_argument&) { rejected = true; }
                        require(rejected && bytes(restored) == checkpoint,
                            "Invalid caster recovery accepted or mutated installed campaign");
                    };
                    for (const auto [offset, value] : std::array<std::pair<size_t, uint64_t>, 6>{
                        {{96, 0}, {96, 3}, {96, 1}, {104, 0}, {104, 2}, {104, 3}}})
                    {
                        auto invalid = checkpoint;
                        replaceWord(invalid, effectOffset + offset, value);
                        rejectRecovery(std::move(invalid));
                    }
                    // Death is immediately before the projectile/effect counts and one timed effect.
                    const auto deathOffset = effectOffset - 16 - 40;
                    for (const auto [offset, value] : std::array<std::pair<size_t, uint64_t>, 3>{
                        {{16, 0}, {24, 1}, {32, 2}}})
                    {
                        auto invalid = checkpoint;
                        replaceWord(invalid, deathOffset + offset, value);
                        rejectRecovery(std::move(invalid));
                    }
                    auto next = service.prepareNativeTick(authority, id<ServerTick>(state.tick + 1), 1.f/30, {});
                    auto replay = restored.prepareNativeTick(authority, id<ServerTick>(state.tick + 1), 1.f/30, {});
                    require(next && replay && next->commit(accepted) == CanonicalDurabilityResult::Committed
                        && replay->commit(accepted) == CanonicalDurabilityResult::Committed
                        && bytes(service) == bytes(restored), "Old-life caster replay diverged");
                    std::cout << "durable caster: NPC strike/death/respawn old-life effect retained; restart exact; malformed recovery atomic\n";
                    return;
                }
        ...
        if (constantEffects)
        {
            ...
            require(state.timedEffects.size() == (generalConstants ? 21 : 3)
                && std::ranges::all_of(state.timedEffects, [&](const auto& effect) {
                    return effect.sourceKind == 3 && (generalConstants || effect.magnitude == 7)
            ...
            const size_t effectStart = size_t(state.inventory.data()
                - reinterpret_cast<const char*>(started.data())) - state.timedEffects.size() * (generalConstants ? 96 : 80);
            ...
            require(std::ranges::none_of(unequipped.timedEffects, [](const auto& effect) {
                return effect.actor == 0 && effect.sourceKind == 3 && effect.argument == 8;
            ...
            if (generalConstants)
            {
                const auto baseline = read(original);
                const auto attribute = [&](ESM::RefId id) {
                    const auto& value = baseline.combat->actors[0][ESM::Attribute::refIdToIndex(id)];
                    return std::max(0.f, value[0] + value[1] - value[3]);
                };
                const float expectedMagicka = passiveMagickaMultiplier * attribute(ESM::Attribute::Intelligence);
                const float expectedFatigue = attribute(ESM::Attribute::Strength) + attribute(ESM::Attribute::Willpower)
                    + attribute(ESM::Attribute::Agility) + attribute(ESM::Attribute::Endurance);
                // Stock attribute changes derive maxima from attributes even when
                // authored NPDT resource values initially differ from those sums.
                require(state.combat->actors[0][9][0] > expectedMagicka
                    && state.combat->actors[0][10][0] > expectedFatigue
                    && std::abs(unequipped.combat->actors[0][9][0] - expectedMagicka) < .0001f
                    && std::abs(unequipped.combat->actors[0][10][0] - expectedFatigue) < .0001f,
                    "Attribute equipment did not restore derived magicka/fatigue maxima on removal");
            }
            ...
            require(std::ranges::count_if(after.timedEffects, [](const auto& effect) {
                return effect.actor == 0 && effect.sourceKind == 3 && effect.argument == 8 && effect.magnitude == 11;
            ...
            require(std::ranges::count_if(direct.timedEffects, [&](const auto& effect) {
                return effect.actor == 0 && effect.sourceKind == 3 && effect.argument == 8 && effect.magnitude == 7
            ...
            if (generalConstants)
            {
                // Every unmodified source retains its original roll/start tick even
                // while another slot is removed and replaced repeatedly.
                for (const auto& effect : state.timedEffects)
                    if (effect.actor != 0 || (effect.argument != 8 && effect.argument != 1 && effect.argument != 2
                            && effect.argument != uint64_t(ESM::Skill::refIdToIndex(ESM::Skill::Axe) + 9)))
                        require(std::ranges::find(direct.timedEffects, effect) != direct.timedEffects.end(),
                            "Unchanged equipment rerolled or changed identity during replacement");
                for (size_t offset : {size_t(80), size_t(88), size_t(8)})
                {
                    auto bad = started;
                    bad[effectStart + offset] = std::byte{127};
                    bool rejectedImage = false;
                    try { InventoryHost invalid(descriptor, testContentManifest(), *registry, *crypto, bad); }
                    catch (const std::invalid_argument&) { rejectedImage = true; }
                    require(rejectedImage, "Forged constant argument, ordinal or magnitude recovered");
                }
                const auto beforeIdle = image();
                InventoryHost idleRestart(descriptor, testContentManifest(), *registry, *crypto, beforeIdle);
                idleRestart.service().synchronizeCells(authority);
                auto idle = service.prepareNativeTick(authority, id<ServerTick>(5), 1.f/30, {});
                auto recoveredIdle = idleRestart.service().prepareNativeTick(authority, id<ServerTick>(5), 1.f/30, {});
                require(idle && recoveredIdle && idle->commit(accepted) == CanonicalDurabilityResult::Committed
                    && recoveredIdle->commit(accepted) == CanonicalDurabilityResult::Committed,
                    "Unchanged constant continuation failed");
                const auto continued = read(image());
                require(continued.timedEffects == direct.timedEffects
                    && continued.combat->actors[0][9][0] == direct.combat->actors[0][9][0]
                    && continued.combat->actors[0][10][0] == direct.combat->actors[0][10][0]
                    && continued.combat->rng == direct.combat->rng
                    && std::ranges::equal(service.inventoryImage(), idleRestart.service().inventoryImage()),
                    "Idle equipment or restart consumed RNG or changed rolled effects");
                const auto invalidView = service.projectInventory(authority, id<SessionId>(1),
                    id<ServerTick>(5), id<CanonicalRevision>(5));
                const auto& inventory = invalidView->playerInventory.front();
                const auto invalidItem = std::ranges::find(inventory.stacks,
                    id<ItemPrototypeId>(MWWorld::inventoryRecordId(ESM::RefId::stringRefId("npc_passive_invalid_shirt"))),
                    &CanonicalItemStack::prototypeId);
                require(invalidItem != inventory.stacks.end(), "Unsupported constant fixture item missing");
                const auto priorInvalid = image();
                auto invalidEquip = service.prepareInventory(authority, bind(authority,
                    command(InventoryTransactionKind::EquipItem, invalidItem->stackId,
                        invalidItem->prototypeId, inventory.revision, 6)).proposal());
                require(bool(invalidEquip), "Unsupported effect fixture did not reach composed validation");
                bool invalidPlan = false;
                try { (void)service.prepareNativeTick(authority, id<ServerTick>(6), 1.f/30, std::move(invalidEquip)); }
                catch (const std::invalid_argument&) { invalidPlan = true; }
                require(invalidPlan && image() == priorInvalid,
                    "Unsupported mixed constant plan mutated equipment, effects or RNG");
                bool hitVerified = false;
                for (uint64_t time = 6; time < 90 && !hitVerified; ++time)
                {
                    const auto prior = read(image());
                    const auto condition = dynamic_cast<InventoryService&>(service).selectedNpcWeaponCondition().value();
                    auto step = service.prepareNativeTick(authority, id<ServerTick>(time), 1.f/30, {});
                    require(bool(step), "Constant combat tick missing");
                    ...
                    require(step->commit([&](auto bytes) {
                        staged.assign(bytes.begin(), bytes.end()); return CanonicalDurabilityResult::Rejected;
                    }) == CanonicalDurabilityResult::Rejected, "Constant combat rejection failed");
                    const auto candidate = read(staged);
                    if (events && !events->actorEvents().empty() && events->actorEvents()[0].hit)
                    {
                        const auto& hit = events->actorEvents()[0];
                        const auto& strength = prior.combat->actors[2][0];
                        float bonus = 0;
                        for (const auto& effect : prior.timedEffects)
                            if (effect.actor == 2 && effect.sourceKind == 3 && effect.argument == 1)
                                bonus += effect.magnitude;
                        const float modified = std::max(0.f, strength[0] - strength[3] + strength[1] + bonus);
                        const float raw = TES3MP::openMwAdjustedWeaponDamage(passiveSettings, modified,
                            float(condition) / defenseWeapon.mData.mHealth, true,
                            defenseWeapon.mData.mChop[0] + (defenseWeapon.mData.mChop[1]
                                - defenseWeapon.mData.mChop[0]) * candidate.melee->state.mStrength);
                        const auto& skill = prior.combat->actors[0][11 + ESM::Skill::refIdToIndex(ESM::Skill::Unarmored)];
                        const float armor = TES3MP::openMwUnarmoredRating(passiveSettings,
                            std::max(0.f, skill[0] - skill[3] + skill[1]));
                        const float expected = std::max(1.f, TES3MP::openMwArmorAdjustedDamage(passiveSettings, raw, armor));
                        require(bonus > 0 && std::abs(hit.damage - expected) < .0001f,
                            "Equipped Strength effects did not feed shared weapon damage");
                        require(candidate.combat->actors[2][0][1] == prior.combat->actors[2][0][1],
                            "Constant combat overlay accumulated in durable stats");
                        hitVerified = true;
                    }
                    require(step->commit(accepted) == CanonicalDurabilityResult::Committed,
                        "Constant combat retry failed");
                }
                require(hitVerified, "No constant-modified melee hit was verified");
                std::cout << "general constants: arguments, duplicate effects, zero/variable rolls, shirt/ring, preserved instances, forged images and restart continuation verified\n";
            }
        ...
        if (melee)
        {
            ...
            if (spell)
            {
                ...
                if (collection)
                {
                    ...
                    {
                        InventoryHost onceHost(descriptor, testContentManifest(), *registry, *crypto, contactImage);
                        auto& once = onceHost.service(); once.synchronizeCells(playersAtLaunch);
                        const auto beforeInventory = once.projectInventory(playersAtLaunch, id<SessionId>(1),
                            id<ServerTick>(castTick), id<CanonicalRevision>(castTick));
                        require(beforeInventory && !beforeInventory->playerInventory.empty(),
                            "CastOnce source inventory missing");
                        const auto& stacks = beforeInventory->playerInventory.front().stacks;
                        const auto source = std::ranges::find(stacks,
                        ...
                        onceUse.expectedInventoryRevision = beforeInventory->playerInventory.front().revision;
                        auto prepared = once.prepareMagicUse(playersAtLaunch, proposal(onceUse), id<ServerTick>(castTick));
                        require(bool(prepared), "CastOnce source rejected before launch");
                        auto launch = once.prepareNativeTick(playersAtLaunch, id<ServerTick>(castTick),
                        ...
                        require(launch->commit([&](auto bytes) {
                            ...
                            && std::ranges::equal(once.inventoryImage(), contactImage),
                            "Rejected CastOnce launch consumed its source");
                        const auto staged = readActorCampaign({reinterpret_cast<const char*>(candidate.data()), candidate.size()});
                        require(staged.projectiles.size() == 1
                            && staged.projectiles.front().source == onceUse.sourceId
                            && staged.projectiles.front().effectSource,
                            "CastOnce launch omitted its durable projectile");
                        require(staged.combat && staged.combat->actors[0][9][2] == contacted.combat->actors[0][9][2],
                            "CastOnce launch spent magicka");
                        const auto projected = once.projectInventory(playersAtLaunch, id<SessionId>(1),
                        ...
                        for (uint64_t time = castTick + 2; time < castTick + 90 && !resolved; ++time)
                        {
                            ...
                            const auto event = flight.projectCombatEvents(playersAtLaunch, id<SessionId>(1),
                                id<ServerTick>(time), id<CanonicalRevision>(time), step.get());
                            require(step->commit(accepted) == CanonicalDurabilityResult::Committed,
                                "CastOnce flight failed to commit");
