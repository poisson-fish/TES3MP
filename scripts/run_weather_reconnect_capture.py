...
def _pack_options(pack: Path) -> list[str]:
    result = []
...
def client_command(args: argparse.Namespace, pack: Path, port: int, password: Path,
    ...
    if role not in ROLES:
        raise ValueError(f"unknown weather role: {role}")
    command = [
    ...
    command.extend(_pack_options(pack))
...
def _completion(path: Path, role: str) -> tuple[dict, list[dict]]:
    ...
    if completed[0].get("success") is not True:
        raise RuntimeError(f"desktop weather role failed: {completed[0]!r}")
    return completed[0], records


def _runtime_config(pack: Path, output: Path, port: int, password: Path) -> Path:
    omitted = {"inventory_content_file", "combat_content_file", "script_package_file"}
    replacements = {
    ...
    for raw in pack.joinpath("server.cfg").read_text(encoding="utf-8").splitlines():
        ...
        elif key.endswith("_content_file"):
...
def _seed_established_players(args: argparse.Namespace, pack: Path, phase: Path,
    ...
    template_path = args.resources / "vfs" / "tes3mp" / "players.txt"
    ...
    for index, role in enumerate(roles):
        credential = hashlib.sha256(("weather-credential:" + role).encode()).digest()
        ...
        old_name = tokens[-1]
        new_name = ("weather" + str(index + 1)).encode().hex()
        tokens = [new_name if token == old_name else token for token in tokens]
        records.append(" ".join(tokens))
    with phase.joinpath("players.txt").open("w", encoding="utf-8", newline="\n") as identities:
        identities.write("TES3MP_PLAYER_IDENTITIES_V5\n" + "\n".join(records) + "\n")
...
def _run_phase(args: argparse.Namespace, pack: Path, password: Path, artifacts: Path,
    ...
    _seed_established_players(args, pack, phase, roles)
    server = None
    for _ in range(5):
        port = _free_port()
        config = _runtime_config(pack, phase, port, password)
    ...
    try:
        for role in roles:
            ...
            processes.append(subprocess.Popen(client_command(args, pack, port, password, role, evidence),
                                              stdout=stdout, stderr=stderr, env=_client_environment(args)))
            if len(roles) > 1:
                time.sleep(0.5)
        deadline = time.monotonic() + timeout
        while any(process.poll() is None for process in processes):
            if time.monotonic() >= deadline:
                raise RuntimeError(f"weather roles timed out: {roles!r}")
            if sample_rss:
        ...
        if any(process.returncode != 0 for process in processes):
            raise RuntimeError(f"weather client failed: roles={roles!r} codes={[p.returncode for p in processes]!r}")
    finally:
    ...
    records = {}
    for role in roles:
        complete, role_records = _completion(phase / f"{role}.ndjson", role)
...
def _bake(args: argparse.Namespace, root: Path) -> Path:
    ...
    world = source.joinpath("vanilla-world.txt")
    ...
    text = text.replace("weather_region 2 1 9000 300 1 2", "weather_region 2 1 600 60 1 2")
...
def main() -> int:
    ...
    if args.artifacts.exists() and any(args.artifacts.iterdir()):
        raise RuntimeError(f"artifact directory is not empty: {args.artifacts}")
    ...
    with tempfile.TemporaryDirectory(prefix="tes3mp-weather-") as temporary:
        ...
        path = args.artifacts / "summary.json"
        path.write_text(json.dumps(summary, separators=(",", ":")) + "\n", encoding="utf-8")
        for artifact in args.artifacts.rglob("*"):
